...
if($catid!=''){
$objcon->getList(" AND cat_id IN ('$catid') ",' ORDER BY modifydate DESC ',' LIMIT 0,10');
}else{
$objcon->getList("",' ORDER BY modifydate DESC ',' LIMIT 0,10');
}
$item_r = $objcon->Fetch_Assoc();
$imgs=stripslashes($item_r["thumb_img"]);
$title = Substring(stripslashes($item_r["title"]),0,10);
$intro = Substring(strip_tags(stripslashes($item_r["intro"])),0,20);
