    <?php
    $strWher='ORDER BY `view` DESC ';
    $name_obj='';
    if(isset($viewtype) AND $viewtype=='block'){
        $name_obj=$name;
        $strWher="WHERE sub_id='$sub_id' ORDER BY `view` DESC ";
    }
    ...
    if($objdata->Num_rows()>0){
        ...
        $i='';
        if($objdata->Num_rows()>0){
            echo "<ul class='menu box-item'><h2>Phần mềm ".$name_obj."</h2>";
            while($rows=$objdata->Fetch_Assoc())
            {	$i++;
                $id=$rows["id"];
                $name=$rows["name"];
    ...
    $sql="SELECT `tbl_member_profile`.`first_name`,`tbl_member_profile`.`username`, `tbl_member_profile`.`last_name`, `tbl_member_profile`.`avata`, `tbl_member_profile`.`email`
    ...
    echo "<ul class='menu box-item'><h2>Tác giả</h2>";
        ...
        {	$i++;
            ...
            $str="WHERE sub_id='$sub_id'";
            $url=ROOTHOST."tac-gia/".$username.".html";
            $img=$rows['avata'];
            ?>
            <li>
                ...
                <a href='<?php echo $url;?>' title="<?php echo $name;?>" class="txt-author"><?php echo $name;?></a>
                <p>Level: Giảng viên</p>
            </li>
        <?php
        }
    echo "</ul>";
