...
class CLS_MENUFOOD{
	...
	public function Add_new(){
		$sql="INSERT INTO `tbl_food_menu` (`food_id`, `code`, `name`,`thumb`, `cdate`, `isactive`) VALUES ";
		$sql.="('".$this->Food_ID."','".$this->Code."','".$this->Name."','".$this->Thumb."','".$this->Cdate."','".$this->isActive."')";
		return $this->objmysql->Exec($sql);
	}
	public function Update(){
		$sql="UPDATE `tbl_food_menu` SET  
	...
	public function Delete($ids){
		$sql="DELETE FROM `tbl_food_menu` WHERE `id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	public function setActive($ids,$status=''){
		$sql="UPDATE `tbl_food_menu` SET `isactive`='$status' WHERE `id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `tbl_food_menu` SET `isactive`=if(`isactive`=1,0,1) WHERE `id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	public function Order($ids,$order){
		$sql="UPDATE tbl_food_menu SET `order`='".$order."' WHERE `id`='".$ids."'";	
		return $this->objmysql->Exec($sql);
	}
