...
/*   ->setCellValue('G1', 'Đã chi')
   ->setCellValue('H1', 'Công nợ');*/
   $hang = 2;

$obj->Query($sql);
$str_data="BẢNG THỐNG KÊ HOA HỒNG CHIA LÃI ĐỊNH KỲ\n";
...
while($row=$obj->Fetch_Assoc()){
  ...
  /*  $money_used = $objmem->Count_money_hh_wallet(2,$table,$row['username']);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
      ...
      /*  ->setCellValue('G' . $hang, number_format($money_used))
        ->setCellValue('H' . $hang, number_format($money_have));*/
    $hang++;
}
...
header("Content-Disposition: attachment; filename=\"hh_chialaidinhky.xlsx\"");
