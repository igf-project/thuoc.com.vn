...
class CLS_DISTRICT{
    ...
    public function getList($where=' ',$order=' ORDER BY `id` ASC ',$limit=' '){
            $sql="SELECT * FROM `tbl_district` WHERE isactive=1 ".$where.$order.$limit;
    ...
    function getListCbItem($getId='', $swhere='', $arrId=''){
        $sql="SELECT id,name,code FROM tbl_district  WHERE 1=1 ".$swhere." AND `isactive`='1' ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
