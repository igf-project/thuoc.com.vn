...
class CLS_USERS{
	...
	function getMemberByID($memid){
		$sql="SELECT * FROM `tbl_user` WHERE `mem_id`=\"$memid\" ";
	...
	function Add_new(){
		$sql="INSERT INTO `tbl_user` (`username`,`password`,`firstname`,`lastname`,`birthday`,`gender`,`address`,`phone`,`mobile`,`email`,`joindate`,`gmem_id`,`isactive`) VALUES ";
	...
	function setActive($ids,$status=''){
		$sql="UPDATE `tbl_user` SET `isactive`='$status' WHERE `mem_id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `tbl_user` SET `isactive`=if(`isactive`=1,0,1) WHERE `mem_id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	function Delete($memid){
		$sql="DELETE FROM `tbl_user` WHERE `mem_id` in ('$memid')";
		return $this->objmysql->Query($sql);
	}
}
?>