...
class CLS_CATEGORY{
    ...
    public function listTable($strwhere="",$page=1,$parid=0,$level=0,$rowcount){
        ...
        $leng=MAX_ROWS;
        $sql="SELECT * FROM tbl_category WHERE 1=1 $strwhere AND par_id=$parid ORDER BY `order` ASC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $str_space="";
        if($level!=0){  
    ...
    public function Add_new(){
        $sql=" INSERT INTO `tbl_category`(`par_id`,`name`,`code`,`thumb`,`intro`,`type`,`isactive`) VALUES";
