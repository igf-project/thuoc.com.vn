...
class CLS_SPECIALIST{
	...
	public function listTable($strwhere="",$page){
		...
		$leng=MAX_ROWS;
		$sql="SELECT * FROM tbl_specialist $strwhere ORDER BY `id` DESC LIMIT $star,$leng";
	...
	public function Add_new(){
		$sql=" INSERT INTO `tbl_specialist`(`name`,`code`,`thumb`,`intro`,`isactive`) VALUES";
		$sql.="('".$this->Name."','".$this->Code."','".$this->Thumb."','".$this->Intro."','".$this->isActive."')";
	...
	public function Delete($id){
		$sql="DELETE FROM `tbl_specialist` WHERE `id` in ('$id')";
		return $this->objmysql->Exec($sql);
	}
	public function setActive($ids,$status=''){
		$sql="UPDATE `tbl_specialist` SET `isactive`='$status' WHERE `id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `tbl_specialist` SET `isactive`=if(`isactive`=1,0,1) WHERE `id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	public function Order($arr_id,$arr_quan){
		$n=count($arr_id);
		for($i=0;$i<$n;$i++){
			$sql="UPDATE `tbl_specialist` SET `order`='".$arr_quan[$i]."' WHERE `id`='".$arr_id[$i]."'";
			$this->objmysql->Exec($sql);
		}
	}
	/* combo box*/
	function getListCbItem($getId='', $swhere='', $arrId=''){
		$sql="SELECT id,name,code FROM tbl_specialist  WHERE 1=1 ".$swhere." AND `isactive`='1' ORDER BY `name` ASC";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
