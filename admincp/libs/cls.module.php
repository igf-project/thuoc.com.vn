...
class CLS_MODULE{
	...
	public function getList($where='',$limit=''){
		$sql=" SELECT * FROM `view_module` ".$where." ORDER BY `position` ASC, `order` ASC ".$limit;
	...
	public function Add_new(){
		$sql='INSERT INTO `tbl_modules` (`type`,`viewtitle`,`mnu_id`,`cat_id`,`question_group`,`theme`,`position`,`mnuids`,`class`,`isactive`) VALUES ';
		$sql.="('".$this->pro['Type']."','".$this->pro['ViewTitle']."','".$this->pro['MnuID']."','".$this->pro['CatID']."','".$this->pro['Question_group']."','".$this->pro['Theme']."','".$this->pro['Position']."','".$this->pro['Mnuids']."','".$this->pro['Class']."','".$this->pro['isActive']."')";
	...
	function Update(){
		...
		`cat_id`='".$this->pro['CatID']."',
		`question_group`='".$this->pro['Question_group']."',
		`theme`='".$this->pro['Theme']."',
		`position`='".$this->pro['Position']."',
		`mnuids`='".$this->pro['Mnuids']."',
		`class`='".$this->pro['Class']."',
		`isactive`='".$this->pro['isActive']."' 
		WHERE `mod_id`='".$this->pro['ID']."'";
		$this->objmysql->Exec('BEGIN');
		$result=$this->objmysql->Exec($sql);
		
		$sql="UPDATE `tbl_modules_text` SET 
		`title`='".$this->pro['Title']."',
		`content`='".$this->pro['HTML']."' 
		WHERE `mod_id`='".$this->pro['ID']."' AND  `lag_id`='".$this->pro['LangID']."'";
		$result1=$this->objmysql->Exec($sql);
