...
class CLS_GMEMBER{
	...
	public function __set($proname,$value){
		if(!isset($this->pro[$proname])){
			echo ($proname.' is not member of CLS_GMEMBER Class' );
			return;
		}
	...
	public function getList($where='',$limit=''){
		$sql='SELECT * FROM `tbl_gmember` '.$where.' ORDER BY `name` '.$limit;
	...
	function setActive($ids,$status=''){
		$sql="UPDATE `tbl_gmember` SET `isactive`='$status' WHERE `gmem_id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `tbl_gmember` SET `isactive`=if(`isactive`=1,0,1) WHERE `gmem_id` in ('$ids')";
	...
	function Update(){
		$sql="UPDATE `tbl_gmember` SET `par_id`=\"".$this->pro["ParID"]."\",`name`=\"".$this->pro["Name"]."\",`intro`=\"".$this->pro["Intro"]."\",`isadmin`=\"".$this->pro["isAdmin"]."\",`isactive`=\"".$this->pro["isActive"]."\" ";
